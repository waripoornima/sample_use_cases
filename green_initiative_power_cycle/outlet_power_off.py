"""
    ...
    This will help to save the power by turning off the power when reservation ends
...
# toggle poer only in if you found the tbml_string
if tbml_string:
    # parse the topology
    ...
    for resource in topology_parse.resource_list:
        ...
        if power_managed.lower() == 'true':
            ...
            outlet_off_cli = 'olOff ' + outlet_number + '\n'
            ...
            if 'outlet '+outlet_number+': on' in show_status.lower():
                logger.info(f'Turning off outlet {outlet_number}')
                device_connection.send(outlet_off_cli)
                sleep(1)
                show_status = device_connection.recv(12000).decode('utf-8')
                if 'success' in show_status.lower():
                    logger.info(f'Successfully Turned Off the outlet {outlet_number}')
                else:
                    logger.warning(f'Failed to Turn Off the Outlet {outlet_number}')
            ...
            logger.info(f'resource {resource} is not PDU Managed')